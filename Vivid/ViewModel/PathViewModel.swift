                ...
                for record in records {
                    ...
                    if let arrayOfData = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(drawingBytes) as? [Data] {
                        if let clocation = record["location"] as? CLLocation {
                            ...
                            print("S:\(clocation.coordinate.latitude)")
                            print("S:\(clocation.coordinate.longitude)")
...
//                            let distance = self?.haversineDistanceInMeters(from:  currentClLocation.coordinate, to: clocation.coordinate)
                            ...
                            let copDistance: Double  = 915 * 3 // meters
                            print("d:\(distance)")
...
//        print("d:1:\(")
        ...
        let clLocation = CLLocation(coordinate: coordinate, altitude: 4.0, horizontalAccuracy: 0.9, verticalAccuracy: 0.9, course: locationManager.currentLocation.location?.course ?? 0.9, courseAccuracy: locationManager.currentLocation.location?.courseAccuracy ?? 0.9, speed: locationManager.currentLocation.location?.speed ?? 0.9, speedAccuracy: locationManager.currentLocation.location?.speedAccuracy ?? 0.9, timestamp: locationManager.currentLocation.location?.timestamp ?? .now)
//        let arAnchor = ARAnchor
            pathRecord["location"] =  clLocation
